...
{{--                                        <dd class="col-sm-9">{{ $product->sku }}</dd>--}}
                                    ...
                                    <div class="mb-3">
                                        @if ($product->special_price > 0)
                                            <var class="price h3 " style="color: #c66">
                                                <span class="num" id="productPrice">{{ $product->special_price }}</span>&nbsp;<span class="currency">AMD</span>
                                                &nbsp;&nbsp;&nbsp;
                                                <del class="price-old"> {{ $product->price }}AMD</del>
                                            </var>
                                        @else
                                            <var class="price h3 " style="color: #c66">
                                                <span class="num" id="productPrice">{{ $product->price }}</span>&nbsp;<span class="currency">AMD</span>
                                            </var>
                                        @endif
                                    </div>
                                    <hr>
                                    <form action="{{ route('product.add.cart') }}" method="POST" role="form" id="addToCart">
                                        ...
                                        <hr>
                                        <button type="submit" class="btn btn-dark" style="background-color: #c66;border: rgb(250, 250, 250);"><i class="fas fa-shopping-cart"></i> Add to Cart</button>&nbsp;&nbsp;
                                        <a href="{{ route('home') }}" class="btn btn-dark" style="border: rgb(250, 250, 250);"><i class="fas fa-plus-square"></i> Continue to buy</a>
    ...
    <script>
        $(document).ready(function () {
            $('#addToCart').submit(function (e) {
                if ($('.option').val() == 0) {
                    e.preventDefault();
                    swal('It is mandatory to choose an attribute');
                }
            });
